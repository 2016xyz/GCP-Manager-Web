...
CHANGELOG = [
    ...
    },
    {
        ...
        "date": "2026-09-26",
        "notes": [
            "按使用反馈的 4 项改进 + 顺带查出的 2 个前端缺陷。全部在真实浏览器"
            "会话上验证过，测试从 606 项增至 620+ 项",
            "★ 操作审计改服务端分页：原来是「一次拉最近 200 条」，库一大就是纯粹"
            "浪费，而且干脆看不到 200 条以前的历史。现默认每页 5 条（可选 "
            "5/10/20/50），支持上一页/下一页/跳页，由 SQL 层 LIMIT/OFFSET 分页，"
            "响应带 total/page/pages。旧的 ?limit=N 调用保持兼容",
            "★ 账号管理页可单独测试代理是否有效：原来只有「测试连通性」，它验的是"
            "「密钥+网络」整体能不能列出实例，失败时分不清是密钥错还是代理挂。"
            "新增「测代理」按钮，只经该代理发一次轻量请求，回显可用状态、延迟、"
            "具体报错，并能区分「代理不可达」与「代理活着但被目标拦截」。"
            "探测目标地址是硬编码常量，不接受请求方传入（否则就是 SSRF）",
            "★ 命令执行页显示机器列表：原来页面只有「全部实例 / 仅勾选实例」两个"
            "单选项，却**没有任何地方能勾选实例** —— 勾选状态只存在于实例列表页，"
            "进到这里看不到机器、也不知道会执行到哪几台。现补上完整目标列表"
            "（勾选状态两页互通），并提供「全选/全不选/只选运行中」与空态引导",
            "★ 实例状态改中文显示：RUNNING / PROVISIONING / TERMINATED 这些是 "
            "GCP API 的枚举值，直接摆给用户看不直观。现映射为「运行中 / 创建中 / "
            "已终止」等 9 种中文，原文保留在 title 里便于对照 API 与日志。"
            "实例列表页、命令执行页、GCP 资源勘察弹窗三处统一",
            "★ 顺带修复（测试中新发现）代理密码明文泄漏：mask_proxy 只处理 "
            "scheme://user:pass@host 一种写法，只要字符串里没有 @ 就原样返回 —— "
            "而本产品文档里明确支持的 host:port:user:pass 写法没有 @，录入后"
            "被原样存库、也原样回显到账号列表，等于把代理密码明文摆在页面上"
            "（而页面提示文案却写着「代理密码在列表中已打码」）。现补上该形态"
            "的打码，实测 1.2.3.4:8080:user:secretPw → 1.2.3.4:8080:user:***",
            "★ 顺带修复（测试中新发现）审计表格时间列显示原始时间戳：options 对象里 "
            "fmtTime **定义了两次**，后者覆盖前者 —— 生效的那份按字符串处理"
            "（Date.parse），于是所有秒级数字时间戳（审计/任务/会话/日志/最后登录）"
            "Date.parse 得到 NaN 后直接原样返回，页面上显示成 1790410050.5641239。"
            "现合并为一个同时支持「秒级数字」与「ISO 8601 字符串」的函数，"
            "解析不了则原样回显（不伪造时间）",
        ],
    },
    {
